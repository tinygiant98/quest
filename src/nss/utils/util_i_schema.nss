...
/* THIS SHIT WORKS! BUT STILL NEEDS WORK...
WITH RECURSIVE schema_tree AS (
    SELECT
        json_each.key,
        json_extract(json_each.value, '$.type') AS type,
...
SELECT
	...
	CASE
		...
		WHEN schema_tree.type = 'array' THEN '[]'
	END AS value
FROM
    schema_tree
WHERE schema_tree.key IN (SELECT key FROM json_each(json_extract((SELECT schema FROM quest_schema LIMIT 1), '$.fields')));
*/