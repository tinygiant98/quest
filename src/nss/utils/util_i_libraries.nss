...
sqlquery SqlPrepareQuery(object oObject, string sQuery)
{
    return SqlPrepareQueryObject(oObject, sQuery);
    //return SqlPrepareQueryCampaign("CORE_FRAMEWORK_1", sQuery);
}
...
void CreateLibraryTable(int bReset = FALSE)
{
    ...
    if (bReset)
    {
        ...
        sqlquery sqlDrop = SqlPrepareQuery(GetModule(), sDrop);
        SqlStep(sqlDrop);
    }
    ...
    sqlquery sql = SqlPrepareQuery(GetModule(), sLibraries);
    SqlStep(sql);
}
...
void AddLibraryScript(string sLibrary, string sScript, int nEntry)
{
    ...
    sqlquery sql = SqlPrepareQuery(GetModule(), sQuery);
...
void LoadLibrary(string sLibrary, int bForce = FALSE)
{
    ...
    {
        Notice("Library not loaded, loading now.");
